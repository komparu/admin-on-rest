    ...
    return fetch(url, (0, _extends3.default)({}, options, { headers: requestHeaders })).then(function (response) {
        ...
        if (status < 200 || status >= 300) {
            if (json && json.description) {
                return Promise.reject(new _HttpError2.default(json.description, status));
            } else if (json && json.message) {
                return Promise.reject(new _HttpError2.default(json.message, status));
            }
            return Promise.reject(new _HttpError2.default(statusText, status));
        }
        return { status: status, headers: headers, body: body, json: json };
    }).catch(function (err) {
        return Promise.reject(new _HttpError2.default(err.message, err.status));
    });
};
